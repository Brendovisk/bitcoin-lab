...
function exec(cmd: string): Line[] {
  ...
  if (c === "getblockcount") return [{ kind: "out", text: String(840219 + Math.floor(Math.random() * 8)) }];
  ...
  if (c.startsWith("generatetoaddress")) {
    ...
    return [{ kind: "out", text: `[ ${Array.from({ length: Math.min(n, 5) }, () => `"${rand(64)}"`).join(", ")}${n > 5 ? ", …" : ""} ]` }];
  }
  if (c === "getrawmempool") {
    return [{ kind: "out", text: `[ ${Array.from({ length: 4 }, () => `"${rand(64)}"`).join(", ")} ]` }];
  }
  ...
  return (
    <div className="border border-border/70 bg-background/80 font-mono text-sm">
      ...
      <div id="term-scroll" className="p-4 h-[420px] overflow-auto space-y-1">
        {lines.map((l, i) => (
          <pre key={i} className={
            l.kind === "in" ? "text-bitcoin" :
            l.kind === "err" ? "text-destructive" :
            l.kind === "info" ? "text-muted-foreground" : "text-foreground"
          }>{l.text}</pre>
        ))}
      </div>
      <form
