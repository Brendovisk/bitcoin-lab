...
const rand = (n: number) => Array.from({ length: n }, () => HEX[Math.floor(Math.random() * 16)]).join("");
  ...
  return (
    <div className="border border-border/70 bg-card/30 p-6 space-y-6">
      <div className="flex items-end justify-between gap-6 flex-wrap">
        <div>
          <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground mb-1">prova de trabalho</div>
          <h3 className="font-display text-2xl">Encontrar um hash que começa com <span className="text-bitcoin">{difficulty}</span> zero(s)</h3>
        </div>
      ...
      <div className="border border-border/60 p-4 bg-background/60">
        <div className="text-[10px] text-muted-foreground uppercase tracking-widest mb-2">candidato</div>
        <div className="font-mono text-sm break-all">
          <span className={hash.startsWith(target) ? "text-signal-glow" : "text-bitcoin"}>{hash.slice(0, difficulty)}</span>
      ...
      <p className="text-sm text-muted-foreground leading-relaxed">
        Mineradores chutam números (<span className="text-bitcoin">nonce</span>) até achar um hash com zeros suficientes na frente.
        Quanto mais zeros o protocolo exige, mais raro é o hash — e mais energia custa achar um.
      </p>
    </div>
  );
}
