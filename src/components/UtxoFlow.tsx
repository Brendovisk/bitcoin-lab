...
export function UtxoFlow() {
  ...
  function spend() {
    ...
    setHistory((h) => [`gastou ${selected.length} utxo(s) → ${target} sat + troco ${change} sat`, ...h].slice(0, 5));
    setSelected([]);
  }
    ...
    <div className="grid grid-cols-12 gap-6">
      <div className="col-span-8 border border-border/70 bg-card/30 p-6">
        <div className="flex justify-between mb-4">
          <div>
            <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">sua carteira</div>
            <div className="font-display text-xl">Pedaços de moeda</div>
          </div>
          <div className="font-mono text-xs text-muted-foreground">
            {utxos.length} utxos · <span className="text-bitcoin">{utxos.reduce((s, u) => s + u.sats, 0).toLocaleString()} sats</span>
          </div>
        </div>
          ...
          {utxos.map((u) => {
            ...
            return (
              ...
              >
                ...
                <div className="font-mono text-[10px] text-muted-foreground">{u.sats.toLocaleString()} sat</div>
        ...
        {history.length > 0 && (
          <div className="mt-6 pt-4 border-t border-border/40 font-mono text-[11px] text-muted-foreground space-y-1">
            <div className="text-muted-foreground/60 uppercase tracking-widest text-[9px] mb-2">// histórico</div>
      ...
      <aside className="col-span-4 border border-border/70 bg-card/30 p-6 space-y-5">
        <div>
          <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">construir transação</div>
        ...
        <p className="text-xs text-muted-foreground leading-relaxed">
          Você não “tira sats” de uma carteira. Você pega pedaços inteiros, gasta tudo, e o que sobra volta como <span className="text-bitcoin">troco</span>.
        </p>
      </aside>
    </div>
  );
