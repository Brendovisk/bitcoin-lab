...
async function getTx(txid: string): Promise<MempoolTx | null> {
  try {
    const [entry, raw] = await Promise.all([
      regtest.call<{ vsize: number; fees: { base: number } }>('getmempoolentry', [txid]),
      regtest.call<RawTransaction>('getrawtransaction', [txid, true]),
    ]);
...
async function getLatestTxs(count = 8): Promise<MempoolTx[]> {
  const mempool = await regtest.call<RawMempoolVerbose>('getrawmempool', [true]);
